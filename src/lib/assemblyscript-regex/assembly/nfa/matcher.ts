...
export class Matcher {
  ...
  static fromCharacterClassNode(node: CharacterSetNode, flags: Flags): CharacterSetMatcher {
    return new CharacterSetMatcher(node.charClass, flags.dotAll);
  }

  static fromCharacterRangeNode(node: CharacterRangeNode, flags: Flags): CharacterRangeMatcher {
    return new CharacterRangeMatcher(new Range(node.from, node.to), flags.ignoreCase);
  }

  static fromCharacterSetNode(node: CharacterClassNode, flags: Flags): CharacterClassMatcher {
    Matcher._flags = flags;
    const matchers = node.expressions.map<Matcher>((exp) => {
      switch (exp.type) {
        case NodeType.CharacterRange:
          return Matcher.fromCharacterRangeNode(exp as CharacterRangeNode, Matcher._flags);
        case NodeType.Character:
          return Matcher.fromCharacterNode(exp as CharacterNode, Matcher._flags);
        case NodeType.CharacterSet:
          return Matcher.fromCharacterClassNode(exp as CharacterSetNode, Matcher._flags);
        default:
  ...
  static fromCharacterNode(node: CharacterNode, flags: Flags): CharacterMatcher {
  ...
  matches(code: u32): bool {
    switch (this.charClass) {
      ...
      case Char.Dot:
        return this.dotAll
          ? true
          : code != Char.CarriageReturn && code != Char.LineFeed && code != 8232 && code != 8233;
      case Char.w:
      ...
      default:
        throw new Error("unsupported character class - " + String.fromCharCode(this.charClass));
    }
  }
}
