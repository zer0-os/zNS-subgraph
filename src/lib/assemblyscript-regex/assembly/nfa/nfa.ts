...
export class State {
  ...
  constructor(public transitions: State[] = [], public id: u32 = State._stateId++) {}
...
export class GroupEndMarkerState extends State {
  ...
  matches(input: string, position: u32): MatchResult {
    if (this.startMarker.capturing) {
      this.startMarker.capture = input.substring(this.startMarker.location, position);
    }
    return MatchResult.Ignore;
  }
}
...
export class MatcherState<T extends Matcher> extends State {
  ...
  matches(input: string, position: u32): MatchResult {
    return this.matcher.matches(input.charCodeAt(position)) ? MatchResult.Match : MatchResult.Fail;
  }
}

export class Automata {
  static toNFA(ast: AST, flags: Flags): Automata {
    let factor = new AutomataFactor(flags);
    return factor.automataForNode(ast.body);
  }
  ...
  automataForNode(expression: Node | null): Automata {
    ...
    switch (expression.type) {
      ...
      case NodeType.Alternation: {
        ...
        return union(this.automataForNode(node.left), this.automataForNode(node.right));
      }
      case NodeType.CharacterClass:
        return Automata.fromMatcher(
          Matcher.fromCharacterSetNode(expression as CharacterClassNode, this.flags),
        );
      case NodeType.CharacterSet:
        return Automata.fromMatcher(
          Matcher.fromCharacterClassNode(expression as CharacterSetNode, this.flags),
        );
      case NodeType.Group: {
        const node = expression as GroupNode;
        return group(this.automataForNode(node.expression), node.capturing, node.id);
      }
